...
//      closeLoad(context);
    ...
    String pwd = isCodeLogin?_passwordController.text:encodeBase64(_passwordController.text);

    try {
      User user = await AppRepository.fetchLogin(isCodeLogin?"code":"password",name, pwd);
  ...
  Widget buildChangeLogin() {
    return GestureDetector(
      onTap: () {
        ...
        _nameController.value = new TextEditingValue(text: SpUtil.getString("loginphone"));
      },
      child: Text(
...
//            Navigator.of(context).push(new MaterialPageRoute(builder: (context) => WebViewPage(Api.USER_AGREEMENT,"用户协议")));
  ...
  _buildBody(){
    return Padding(
      padding: EdgeInsets.only(left: 20.0, right: 20.0, top: ScreenUtil.getStatusBarH(context)+20.0),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.center,
        children: <Widget>[
          LoadImage("login/logo",height: 150.0,),
          Gaps.vGap16,
          MyTextField(
            key: const Key('phone'),
            ...
            child: MyTextField(
              ...
              getVCode: () async{
                if (_nameController.text.length == 11){
                  String name = _nameController.text;
                  try {
                    bool isSendSuccess = await AppRepository.fetchGetCode(name, "LOGIN");
                    if(isSendSuccess){
                      Toast.show("获取成功");
                      return true;
                    }else{
